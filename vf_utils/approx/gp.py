...
class GPApproximator(VectorFieldApproximator):
	...
	def __init__(self):
		...
		#self._multKernel = GPy.util.multioutput.ICM(input_dim=2,num_outputs=2,kernel=self._K)

		self._gpModelX = None
		self._gpModelY = None

		# GPy.util.multioutput.ICM(input_dim=2,num_outputs=2,kernel=GPy.kern.RBF(1))
	...
	def approximate(self):
		...
		#self._gpModel = GPy.models.GPCoregionalizedRegression([x, x], [y1, y2], self._multKernel)
		...
		return (self._gpModelX, self._gpModelY)